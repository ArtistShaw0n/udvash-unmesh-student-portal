...
function GroupRow({ top, c, ini, name, prev, time, badge, pin }: Row) {
  return (
    <>
      ...
      <p className="absolute left-[72px] truncate font-['Inter',sans-serif] text-[12px] leading-[20px] text-[rgba(97,97,97,0.8)] dark:text-[rgba(232,232,232,0.8)]" style={{ top: top + 27, right: pin ? 90 : 46 }}>{prev}</p>
      ...
      {badge && (
        <span className="absolute flex h-[18px] min-w-[18px] items-center justify-center rounded-[18px] bg-[#00a511] px-[3px] font-['Inter',sans-serif] text-[12px] font-medium leading-[normal] text-white" style={{ top: top + 28, right: pin ? 50 : 24 }}>{badge}</span>
      )}
      {pin && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={`${CM}/pin.svg`} alt="" aria-hidden="true" className="absolute right-[24px] h-[18px] w-[18px]" style={{ top: top + 28 }} />
      )}
      {/* eslint-disable-next-line @next/next/no-img-element */}
