...
/*
 * Figma V2 — Home screen. light node 1:9720 · dark node 1:10034.
 *   header (50) + 2-col tile grid (cards 176×60, row step 72 from y70) + footer (y746).
 *   tile: icon 36×36 (left 8, top 12) + label Inter Medium 14px #616161/#e8e8e8.
 *   tinted icons sit on a color@10% rounded-[5px] box; plain icons are full 36px.
 *   Live Class/Exam carry a "1 Live, 2 Upcoming" subtitle (#fc5a5a 10px);
 *   Live Exam has a notification dot.
 */
type Tile = {
  label: string;
  icon: string;
  ...
  return (
    <div className="relative h-[60px] w-[176px] rounded-[10px] bg-white shadow-[0px_0px_5px_0px_rgba(0,0,0,0.1)] dark:bg-[#1a1a1a] dark:shadow-[0px_0px_4px_0px_rgba(255,255,255,0.25)]">
      ...
      <div className="absolute left-[52px] top-0 flex h-full flex-col justify-center">
        <span
          className={`font-['Inter',sans-serif] text-[14px] font-medium whitespace-nowrap text-[#616161] dark:text-[#e8e8e8] ${t.tight ? "tracking-[-0.28px]" : ""}`}
        >
          {t.label}
        </span>
        {t.subtitle && (
